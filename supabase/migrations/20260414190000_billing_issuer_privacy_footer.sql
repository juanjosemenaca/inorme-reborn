-- Pie PDF RGPD: texto opcional por emisor (excepto INORME, bloque fijo por CIF en aplicación).

ALTER TABLE public.billing_issuers
  ADD COLUMN IF NOT EXISTS privacy_footer_text text;

COMMENT ON COLUMN public.billing_issuers.privacy_footer_text IS
  'Texto opcional pie PDF (protección de datos) para emisores distintos de INORME S.L. (CIF B60340601).';

ALTER TABLE public.billing_invoices
  ADD COLUMN IF NOT EXISTS issuer_privacy_footer text;

COMMENT ON COLUMN public.billing_invoices.issuer_privacy_footer IS
  'Copia del pie RGPD del emisor en borrador; inmutable tras emitir. Vacío si el emisor es INORME (texto fijo en cliente).';
