-- Texto libre en cliente (p. ej. varios destinatarios separados por ;) y línea elegida en cada factura (PDF).

ALTER TABLE public.clients
  ADD COLUMN IF NOT EXISTS invoice_addressee_line text;

COMMENT ON COLUMN public.clients.invoice_addressee_line IS
  'Destinatarios de facturación (texto libre; opcionalmente varios separados por ; o salto de línea).';

ALTER TABLE public.billing_invoices
  ADD COLUMN IF NOT EXISTS recipient_addressee_line text;

COMMENT ON COLUMN public.billing_invoices.recipient_addressee_line IS
  'Destinatario de factura elegido para el PDF (snapshot en borrador/emisión).';
...
BEGIN
  ...
  IF OLD.status <> 'DRAFT' THEN
    IF NEW.series_id <> OLD.series_id
      ...
      OR NEW.recipient_addressee_line IS DISTINCT FROM OLD.recipient_addressee_line
