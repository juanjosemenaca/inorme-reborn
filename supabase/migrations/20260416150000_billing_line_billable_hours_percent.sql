-- Porcentaje de horas facturables por línea (p. ej. 80% de las horas registradas). Por defecto 100%.

ALTER TABLE public.billing_invoice_lines
  ADD COLUMN IF NOT EXISTS billable_hours_percent numeric(7,4) NOT NULL DEFAULT 100
  CONSTRAINT ck_billing_line_billable_hours_percent CHECK (
    billable_hours_percent > 0::numeric AND billable_hours_percent <= 100::numeric
  );

COMMENT ON COLUMN public.billing_invoice_lines.billable_hours_percent IS
  'Sobre líneas BILLABLE: % de las horas (cantidad) que entran en el importe. 100 = todas.';
...
BEGIN
  IF NEW.line_type <> 'BILLABLE' THEN
    ...
    NEW.unit_price := 0;
    NEW.irpf_rate := 0;
    NEW.billable_hours_percent := 100;
    NEW.taxable_base := 0;
    NEW.vat_amount := 0;
    NEW.irpf_amount := 0;
    NEW.line_total := 0;
  ...
  v_base := round((NEW.quantity * NEW.unit_price * (NEW.billable_hours_percent / 100.0))::numeric, 2);
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'billableHoursPercent', l.billable_hours_percent,
        'vatRate', l.vat_rate,
