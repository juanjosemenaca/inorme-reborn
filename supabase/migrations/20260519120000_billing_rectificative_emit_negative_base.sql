-- Las rectificativas copian líneas con cantidades negativas → base imponible total < 0.
-- La condición histórica `v_base_total <= 0` impedía emitirlas.
...
BEGIN
  ...
  IF inv.invoice_kind = 'RECTIFICATIVE' THEN
    IF v_base_total = 0 THEN
      RAISE EXCEPTION 'Factura rectificativa sin base imponible en líneas facturables.';
    END IF;
  ELSE
    IF v_base_total <= 0 THEN
...
COMMENT ON FUNCTION public.billing_emit_invoice(uuid, uuid, date) IS
  'Emite borrador: RECTIFICATIVE permite base imponible total distinta de cero (habitualmente negativa).';
